  ...
  // 1.1 — Org context
  {
    ...
    modal: {
      ...
      blocks: [
        ...
        { kind: "text", body: "Without the graph, Rovo is just another AI wrapper. With it, Rovo can do things no one else's AI can — because no one else has this context." },
      ],
  ...
  // 1.3 — Specificity & iteration
  {
    ...
    modal: {
      ...
      blocks: [
        {
          kind: "terminal",
          title: "rovo-dev :: voice-mode",
          cursor: true,
          lines: [
            { type: "comment", text: "▣  Listening… (▍▎▌▎▋▎▌▍▎▌▍▎▋ )" },
            { type: "out", text: "okay so the modal needs to be a bit bigger and the" },
            { type: "out", text: "image should be on the left with the caption underneath" },
            { type: "blank" },
            { type: "prompt", user: "rovo", cmd: "→ updating App.tsx · 3 files" },
            { type: "out", text: "✓ patched modal layout" },
            { type: "out", text: "✓ moved image-left" },
            { type: "out", text: "✓ caption below" },
            { type: "blank" },
            { type: "prompt", user: "milly", cmd: "yeah perfect, now make the green a bit brighter" },
          ],
        },
      ],
  ...
  // 3.3 — Hard easier
  {
    ...
    modal: {
      ...
      blocks: [
        ...
        { kind: "text", body: "Three agents in a quality gate: Synthesizer reads, Auditor verifies, Critic finds the gaps. If quote accuracy drops below 80%, the report is flagged as unreliable before it ever lands on a desk." },
      ],
  ...
  // 4.2 — Pen
  {
    ...
    modal: {
      ...
      blocks: [
        ...
        { kind: "text", body: "Five sections, three points each, scrawled before any pixel was placed. The paper was thinking — Rovo Dev was building." },
      ],
      ...
      blocks: [
        {
          ...
          cursor: true,
          lines: [
            { type: "prompt", user: "milly", cmd: "tree -L 2" },
            { type: "out", text: "milly-os/" },
            { type: "out", text: "├── people/        # 10 reports · 1-1 notes · growth plans" },
            { type: "out", text: "├── projects/      # 32 active · status · next actions" },
            { type: "out", text: "├── decisions/     # adr-style log of every call I make" },
            { type: "out", text: "├── inbox/         # drop zone — agent triages overnight" },
            { type: "out", text: "├── side-quests/   # writing · talks · experiments" },
            { type: "out", text: "└── agents/        # claws — each owns a slice of the OS" },
            { type: "blank" },
            { type: "prompt", user: "milly", cmd: "claw morning-brief" },
            { type: "out", text: "▸ 4 things needing your decision today" },
            { type: "out", text: "▸ 2 people drafted updates — review queue" },
            { type: "out", text: "▸ 1 side-quest waiting on you (this talk)" },
          ],
        },
        { kind: "text", body: "All markdown. All CLI. Each 'claw' is an agent that owns a slice. OpenClaw is the philosophy feeding Rovo as a second brain." },
      ],
