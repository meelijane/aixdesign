  ...
  // 3.3 — Hard easier
  {
    ...
    modal: {
      ...
      blocks: [
        {
          kind: "terminal",
          title: "literature-review-agent :: pipeline",
          cursor: false,
          lines: [
            { type: "comment", text: "[You provide: Topic + Keywords + Sources (optional)]" },
            { type: "blank" },
            { type: "prompt", user: "Stage 1", cmd: "Research_Synthesizer → \"State of the Research\"" },
            { type: "out", text: "Reads all sources, extracts direct quotes, classifies evidence" },
            { type: "blank" },
            { type: "prompt", user: "Stage 2", cmd: "Integrity_Auditor → Integrity Audit Report" },
            { type: "out", text: "Independently verifies every quote, checks Jira statuses," },
            { type: "out", text: "catches hallucinations, validates search completeness" },
            { type: "blank" },
            { type: "comment", text: "⚠ QUALITY GATE: If Quote Accuracy < 80%, report flagged as unreliable" },
            { type: "blank" },
            { type: "prompt", user: "Stage 3", cmd: "Insight_Critic → Gap Analysis & Critique" },
            { type: "out", text: "Finds contradictions, sample bias, assigns Research" },
            { type: "out", text: "Maturity Scores per theme, identifies what's missing" },
            { type: "blank" },
            { type: "comment", text: "[Final: Verified Literature Review with confidence ratings]" },
          ],
        },
      ],
